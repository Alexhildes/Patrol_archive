 //Author: Alex Hildebrand   GQ Patrol Engine and Turbo Monitoring  August 2019
...
boolean ignitionOn;
boolean screenReset;     //Is the exhaust temperature over a certain value to warrant turbo timer
int startPin = 26;
...
void ignitionCheck () {
  ...
  if  (ignitionOn == true && screenReset == true && digitalRead(ignitionPin) == HIGH) {
       ...
       return;
          ...
          digitalWrite(screen, HIGH);   //Write high to turn screen on
          ignitionOn = true;
          
          digitalWrite(revCamera, HIGH);  //Write high to turn fuel pump on
          //digitalWrite(auxPump, HIGH);  //Write high to turn fuel pump on
          ...
          //digitalWrite(auxPump, LOW);  //Write high to turn fuel pump off
    
          
    } else if (digitalRead(ignitionPin) == LOW) {

          digitalWrite(screen, LOW);    //Turn off screen, car is off.
          ignitionOn = false;
          screenReset = false;
          digitalWrite(fuelPump, LOW);   //Turn off fuel pump, car is off.
          digitalWrite(revCamera, LOW);  //Turn off camera, car is off.
